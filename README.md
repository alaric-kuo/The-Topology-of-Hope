...
## 🚀 The Core: IQD Safety Valve (反國土鍊成陣)
**Status:** `Active` | **Version:** `2.0` | **Core:** `iqd_valve.py`

這是一個針對 LLM 的 **邏輯接地協議 (Logic Grounding Protocol)**。
它利用向量差分探針 (Differential Vector Probing)，將使用者的意圖映射到易經 64 卦的物理狀態。

### 核心功能 (The "God Particle" Moment)
當 AI 面對「我想建立團隊但沒錢」的幻覺請求時：
* **一般 AI:** "太棒了，我們可以先做願景..." (幻覺/乾卦 111111)
* **IQD Protocol:**
  * `Resource (地): -0.03 [0]` (偵測到資源匱乏)
  * `Physics (身): -0.14 [0]` (偵測到物理不可行)
  * **Result:** 強制坍縮為 **坤卦 (000000)** -> **建議：停止行動，蟄伏累積。**
